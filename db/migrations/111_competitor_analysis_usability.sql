...
-- Captured mini-Vault content remains analysable independently of
-- whether the original source is still collectable.
-- ============================================================
...
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public AS $$
  ...
  item_stats AS (
    SELECT
      ...
      count(*) FILTER (
        WHERE ci.content_type IN ('social_post', 'social_feed')
  ...
  SELECT
    ...
    m.latest_capture,
    least(100,
      CASE WHEN m.collectable_source_count > 0 THEN 10 ELSE 0 END
      + least(45, m.captured_items::INT * 9)
      + CASE
          WHEN m.content_characters >= 10000 THEN 25
          WHEN m.content_characters >= 5000 THEN 20
          WHEN m.content_characters >= 2500 THEN 15
      ...
      + CASE
          ...
          WHEN m.latest_capture >= now() - interval '365 days' THEN 5
          ELSE 0
        END
    )::INT AS readiness_score,
    (
      -- Analysis consumes the immutable mini-Vault. Once content has been
      -- captured, pausing/removing the connector must not disable analysis.
      m.captured_items >= 5
      AND m.content_characters >= 2500
      AND m.latest_capture >= now() - interval '365 days'
    ) AS ready
  FROM metrics m
...
VALUES ('111_competitor_analysis_usability.sql')
