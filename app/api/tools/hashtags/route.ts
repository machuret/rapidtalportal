...
Rules: include the # in each tag. 6-10 broad, 8-12 niche, 4-8 local (empty if no location), 2-4 branded. Realistic, not invented vanity tags.`;
  ...
  const result = await toolJson<Groups>(system, `Topic / niche: ${parsed.data.topic}`, 1200);
  ...
  logToolRun("hashtags", parsed.data.clientId, user.id, parsed.data.topic.slice(0, 80), result.tokens);
