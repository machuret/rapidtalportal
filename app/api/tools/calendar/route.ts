...
export const POST = withAuth(async (req, { user }) => {
  ...
  const system = `You are a social media strategist building a 30-day content calendar.
${contextBits ? `\nThe client:\n${contextBits}\n` : ""}${parsed.data.platform ? `Primary platform: ${parsed.data.platform}.` : "Mix formats across platforms."} Tone: ${parsed.data.tone}.
