...
import { Eye, NotebookPen, Clock, ListChecks, ChevronRight } from "lucide-react";
...
export default async function SupervisionPage() {
  ...
  const [logsRes, timeRes, runsRes] = vaIds.length
    ? await Promise.all([
  ...
  type Stat = { logs: number; hours: number; runs: number; completed: number; lastActive: number };
  const stats: Record<string, Stat> = {};
  const ensure = (id: string) => (stats[id] ??= { logs: 0, hours: 0, runs: 0, completed: 0, lastActive: 0 });
  for (const l of logs) { const s = ensure(l.user_id); s.logs++; s.lastActive = Math.max(s.lastActive, new Date(l.updated_at).getTime()); }
  ...
  const sorted = [...vas].sort((a, b) => (stats[b.id]?.lastActive ?? 0) - (stats[a.id]?.lastActive ?? 0));
    ...
    <div className="max-w-4xl">
      ...
      {sorted.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {sorted.map((va) => {
            const s = stats[va.id] ?? { logs: 0, hours: 0, runs: 0, completed: 0, lastActive: 0 };
            return (
              <Link
                key={va.id}
