                  ...
                  return (
                    <div key={l.id} draggable onDragStart={() => { dragId.current = l.id; }} onClick={() => setEditing(l)}
                      ...
                      <div className="flex items-center gap-2 mt-2 flex-wrap">
                        ...
                        {l.owner_id && (
                          <span className="ml-auto w-5 h-5 rounded-full bg-zinc-700 text-[10px] font-semibold flex items-center justify-center text-zinc-300" title={ownerName(l.owner_id) ?? ""}>
        ...
        <div className="grid md:grid-cols-2 gap-5 mt-2">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex justify-between gap-2 pt-1">
              {mode === "edit" ? (
                <Button variant="destructive" size="sm" onClick={remove} disabled={busy} className="gap-1.5"><Trash2 className="w-3.5 h-3.5" /> Archive</Button>
              ) : <span />}
        ...
        {events.map((e) => (
          <div key={e.id} className={cn("text-sm rounded-lg px-3 py-2", e.kind === "stage" ? "bg-zinc-800/40 text-zinc-400 italic" : "bg-zinc-800/70 text-zinc-200")}>
            <div className="flex items-center justify-between gap-2 mb-0.5">
              <span className="text-[11px] font-medium uppercase tracking-wide text-zinc-500">{KIND_LABEL[e.kind] ?? e.kind}</span>
              <span className="text-[11px] text-zinc-600">{relTime(e.created_at)}</span>
            </div>
