...
export function CoachMemoryPanel({ clientId }: { clientId: string }) {
  ...
  const load = useCallback(async () => {
    try {
      const result = await api.get<{ memories: Memory[] }>(ROUTES.coach.memoriesForClient(clientId), { showErrorToast: false });
      setMemories(result.memories); setFailed(false);
  ...
  async function remove(memory: Memory) {
    ...
    try {
      ...
      await load(); toast.success("The Coach forgot that private memory.");
  ...
  return (
    <div className="surface-card mb-4 overflow-hidden">
      ...
      {open && <div className="border-t border-zinc-800 p-4">
        ...
        {failed && <button type="button" onClick={() => { setLoading(true); void load(); }} className="mb-3 text-xs text-amber-300">Retry private memory</button>}
