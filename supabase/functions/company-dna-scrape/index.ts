...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const crawlRes = await fetch("https://api.firecrawl.dev/v1/scrape", {
      ...
      // waitFor lets JS-heavy pages render; timeout gives slow pages room.
      body: JSON.stringify({
        url,
        formats: ["markdown"],
    ...
    if (!crawlRes.ok || !crawlJson?.data) {
      ...
      const friendly = /timed out|timeout/i.test(String(reason))
        ? "This page took too long to load (heavy JavaScript or a slow site). Try a simpler page on the same domain."
